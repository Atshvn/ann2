...
const TinyMCEComp = ({
    ...
    const handleEditorChange = (item) => {
        ...
        console.log("TinyMCEComp", item.level.content);
    }
            ...
            if (file && file.status === 201) {
                ...
                for (let f = 0; f < dataimg.length; f++) {
                    if (f !== "" && dataimg[f].includes(".jpg")) linkfile = linkfile + "<img width='400' src='" + linkJob + dataimg[f] + "' /> <br/>";
                        ...
                        linkfile += '<span className="mce-preview-object mce-object-video" style="" contenteditable="false" data-mce-object="video" data-mce-p-controls="controls" data-mce-html="' + linkJob + dataimg[f] + '">';
                        linkfile += '<video width="600" height="348" controls="controls">';
                        linkfile += '<source src="' + linkJob + dataimg[f] + '" type="video/mp4"></source>';
                        linkfile += '</video>';
                        linkfile += '<span className="mce-shim"></span></span>';
                    }
                }
            }
        }
    ...
    const fileUpload = async (file) => {
        try {
            ...
            Notification('error',"Upload images fail,pls check")
        }
    }
